...
function _flash(type, msg) {
  ...
  if (type && msg) {
    // Only save to session when setting message
    ...
    if (arguments.length > 2 && format) {
      ...
      msg.forEach(function(val){
        (this.session.flash[type] = this.session.flash[type] || []).push(val);
      });
      return this.session.flash[type].length;
    }
    return (this.session.flash[type] = this.session.flash[type] || []).push(msg);
  } else if (type && this.session.flash) {
